...
RT_paths <- paste("C:Data/2023",RT_files, sep = "/")
...
#Add unique event identifier (run dual_events first) 
visit_data$event_id <- paste(visit_data$feeder, visit_data$start, visit_data$JID, sep = " ")

visit_data_primary <- subset(visit_data, perch == 'primary')
...
#Add column for self-follow: is the individual currently visiting the same individual that just visited the same location before
visit_data_primary$self_follow <- ifelse(visit_data_primary$JID == visit_data_primary$pre_JID, "self", "not self")
...
visit_data_primary$parent <- ifelse(visit_data_primary$pre_JID == visit_data_primary$mother | visit_data_primary$pre_JID == visit_data_primary$father, "parent", "no parent")
...
#Only  (to compare contexts: with bonded adult, with another adult, solo)
...
social_bias_brm1 <- social_bias_brm1_juv_poisson

#Model summary
summary(social_bias_brm1)
...
#Model
social_tolerance_brm4 <- brm(displaced ~ relationship2 + (1|mm(JID, next_JID)) + 
                                   (1|position) + (1|dyad_ID), 
                                   data = visit_data_primary4, 
                                   family = bernoulli(link = "logit"),
                                   prior = c(
...
#Pairwise contrasts for Supp Mat using emmeans
contrasts <- social_tolerance_brm4 %>%
  emmeans(~ relationship) %>%
  contrast(method = "pairwise") %>%
  gather_emmeans_draws() %>%
  median_qi()
...
#(5) MAP ----
feedercoord <- read.csv("Data/feedercoord.csv", header = T, stringsAsFactors = F)
